  ...
  if (limit_time_sig == 1)
  {
      if (key_puppet == 0)
      {
          ...
          else if (key_module == 1)
          {
              sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
                  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
                  cYEL "MOpt-AFL-FAST (core_fuzzing)", use_banner);
          }
          else if (key_module == 2)
          {
              sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
                  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
                  cYEL "MOpt-AFL -FAST(pso_updating)", use_banner);
          }
      }
      else {
          ...
          if (key_module == 0)
          {
              sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
                  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
                  cYEL "MOpt-AFL-FAST + pacemaker (pilot_fuzzing)", use_banner);
          }
          else if (key_module == 1)
          {
              sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
                  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
                  cYEL "MOpt-AFL -FAST+ pacemaker (core_fuzzing)", use_banner);
          }
          else if (key_module == 2)
          {
              sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
                  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
                  cYEL "MOpt-AFL-FAST + pacemaker (pso_updating)", use_banner);
          }
      }
  }
  else {
...
int main(int argc, char** argv) {
  ...
  {    //default L
      ...
      while ((opt = getopt(argc, argv, "+i:o:f:m:t:V:T:L:dnCB:S:M:x:Qp")) > 0)
    switch (opt) {
